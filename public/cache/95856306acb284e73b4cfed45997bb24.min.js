...
+"<div class='row col-sm-12'><a class='btn btn-sm' ng-click='makeAppointment()'>Make Appointment</a></div>",controller:['$scope','$stateParams','utils',function($scope,$stateParams,utils){$scope.cartAdded=false;$scope.cartAdd=function(state){$scope.cartAdded=state;}
$scope.startTime=$scope.selectedTime.time;$scope.endTime=$scope.selectedTime.timeEnd;$scope.makeAppointment=function($element){var isInStorage=false;$scope.$storage.cart.forEach(function(cartItem){if(cartItem.id==$scope.service.id&&cartItem.date==$scope.selectedDate&&api_key==cartItem.api){isInStorage=true;cartItem.date=$scope.selectedDate;cartItem.time=$scope.selectedTime.data;}});if(!isInStorage){$scope.selectedService=utils.findById($scope.services,$scope.service.id);$scope.$storage.cart.push({api:api_key,id:$scope.service.id,date:$scope.selectedDate,dateFormat:moment($scope.selectedDate).format('dddd, DD MMMM YYYY'),time:$scope.selectedTime.data,timeStart:$scope.startTime,timeEnd:$scope.endTime,service:{name:$scope.selectedService.name,price:$scope.selectedService.price}});}
$scope.cartAdd(true);}}]}}}).state('service.cart',{url:'cart',views:{'list':{templateUrl:config.server+'/api/angview/cart?apikey='+api_key,resolve:{serviceRes:function(serviceRes){return serviceRes}},controller:['$scope','$stateParams','filterApi','serviceRes',function($scope,$stateParams,filterApi,serviceRes){$scope.carts=filterApi.filter($scope.$storage.cart,$scope.apikey);$scope.empty=false;if($scope.carts.length>0){$scope.servicesPrice=0;$scope.carts.forEach(function(item){$scope.servicesPrice+=parseInt(item.service.price);})
$scope.tax=Number($scope.servicesPrice*0.1).toFixed(2);$scope.totalPrice=Number($scope.servicesPrice+($scope.servicesPrice*0.1)).toFixed(2);$scope.totalDeposit=Number($scope.servicesPrice*0.2).toFixed(2);$scope.payment={price:$scope.servicesPrice,price_tax:$scope.tax,price_deposit:$scope.totalDeposit,price_total:$scope.totalPrice};}else{$scope.empty=true;}
$scope.submit=function(customer){serviceRes.save($scope.carts,customer,$scope.payment).success(function(data){$scope.$state.transitionTo('service.checkout');}).error(function(data){console.log('ohno');})}}]}}}).state('service.checkout',{url:'checkout',views:{'list':{templateUrl:config.server+'/api/angview/checkout?apikey='+api_key,controller:['$scope','$stateParams','$localStorage',function($scope,$stateParams,$localStorage){$localStorage.$reset({cart:[]});}]}}})}]);$service.controller('datepicker',function($scope,$state){var days=['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];$scope.today=function(){$scope.dt=new Date();};$scope.today();$scope.clear=function(){$scope.dt=null;};$scope.selects=function(date){var selectedDate=moment(date).format('YYYY-MM-DD');$state.go('service.detail.date',{date:selectedDate});};$scope.disabled=function(date,mode){var isDisable=true;for(var i=0;i<$scope.bussinessHour.length;i++){if(days[date.getDay()]==$scope.bussinessHour[i].day){isDisable=false;}}
return(mode==='day'&&isDisable);};$scope.toggleMin=function(){$scope.minDate=$scope.minDate?null:new Date();};$scope.toggleMin();$scope.open=function($event){$event.preventDefault();$event.stopPropagation();$scope.opened=true;};$scope.dateOptions={formatYear:'yy',startingDay:1};$scope.formats=['dd-MMMM-yyyy','yyyy/MM/dd','dd.MM.yyyy','shortDate'];$scope.format=$scope.formats[0];});angular.module('appSys.services.service',[]).factory('services',['$http','utils',function($http,utils){var path=config.apiPath+'services?apikey='+api_key;var contacts=$http.get(path).then(function(resp){return resp.data;});var factory={};factory.all=function(){return contacts;};factory.get=function(id){return contacts.then(function(){return utils.findById(contacts,id);})};return factory;}]).factory('availableTime',['$http',function($http){var path=config.apiPath+''}]).factory('serviceRes',['$http',function($http){return{save:function(data,customer,payment){return $http({method:'POST',url:config.apiPath+'services/checkout?apikey='+api_key,headers:{'Content-Type':'application/x-www-form-urlencoded'},data:{services:data,customer:customer,payment:payment}})}}}]);angular.module('appSys.utils.service',[]).factory('utils',function(){return{findById:function findById(a,id){for(var i=0;i<a.length;i++){if(a[i].id==id)return a[i];}
return null;}};}).factory('filterApi',function(){return{filter:function filter(items,api){var tmp=[];for(var i=0;i<items.length;i++){if(items[i].api==api){tmp.push(items[i]);}}
return tmp;}}});var app=angular.module('appSys',['ui.router','ngAnimate','ngStorage','appSys.services','appSys.services.service','appSys.utils.service','appSys.settings']);app.run(['$rootScope','$state','$stateParams',function($rootScope,$state,$stateParams){$rootScope.$state=$state;$rootScope.$stateParams=$stateParams;}])
