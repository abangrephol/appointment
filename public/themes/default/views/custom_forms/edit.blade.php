{{ Form::model($data,array('route' => array('customform.update',$data->id),'method'=>'POST','class'=>'form form-horizontal')) }}
...
<div class="panel-body">
    ...
    @foreach ( $data->customformfield as $field )
    <div class="form-group">
        {{ Form::hidden('customfieldname[]', $field->name , array('class'=>'form-control')) }}
        {{ Form::hidden('customfieldtype[]', $field->type , array('class'=>'form-control')) }}
        {{ Form::hidden('customfieldreq[]', $field->requirement , array('class'=>'form-control')) }}
        <div class="col-xs-2 col-sm-offset-3 col-xs-offset-2">{{$field->name}}</div>
        <div class="col-xs-2">
            <span class="mr20">{{$fields[$field->type]}}</span>
        </div>
        <div class="col-xs-1">
            <span class="mr20">{{ $field->requirement==0?'optional':'required' }}</span>

        </div>
        <div class="col-xs-1">
            <a class="del-field mr10"><i class="fa fa-trash-o"></i></a>
            <a class="edit-field"  data-field-name="{{$field->name}}" data-field-type="{{$field->type}}" data-field-req="{{$field->requirement}}"><i class="fa fa-edit"></i></a>
        </div>
    </div>
    @endforeach
